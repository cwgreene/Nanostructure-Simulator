...
void pick_up_particle(int part_id, Particles *p_data, int *density, Mesh *mesh)
{
	...
	if(p_data->p_charge[part_id] < 0) //it's an electron
	{
		...
		mesh->electrons_pos[mesh_pos_id].erase(p_data->local_id[part_id]); //unlink particle from local list
	}
	else //it's a hole
	{
		list<int>::iterator it = p_data->local_id[part_id];
		mesh->holes_pos[mesh_pos_id].erase(p_data->local_id[part_id]); //unlink particle from local list
	}
...
{	
	...
	if(p_data->p_charge[part_id] < 0) //it's an electron
	{
		mesh->electrons_pos[mesh_pos_id].push_back(part_id); //unlink particle from local list
		p_data->local_id[part_id] = --(mesh->electrons_pos[mesh_pos_id].end()); //grab iterator position
	}
	else //it's a hole
	{
		mesh->holes_pos[mesh_pos_id].push_back(part_id); //unlink particle from local list
